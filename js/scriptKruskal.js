...
var nodeObj={
	...
	this.input.setAttribute("class", "node" );
	this.input.appendChild( document.createTextNode(a) );
	document.getElementById('nodeList').append(this.input);	}
...
var edgeObj={
	...
	this.id=i+"$";
	this.newrow.setAttribute("class","result-line");
	this.newrow.setAttribute("id", this.id );
	...
	this.input1= document.createElement("label");
	this.input1.setAttribute("id", this.id+"label" );
	this.newrow.append( this.input1 );
	document.getElementById('result').append(this.newrow);
...
function sortEdges()
	...
	newrow.setAttribute("class","result-line");
	 newrow.setAttribute("id", "resultTag$" );
	let input= document.createElement("label");
	input.setAttribute("class", "title" );
	input.appendChild( document.createTextNode("Sorted Edges") );
	newrow.append(input );
	let input1= document.createElement("label");
	input1.setAttribute("id", "resultTag$label" );
	newrow.append( input1 );
	document.getElementById('result').append(newrow);
		...
		{
			edges[i].show();
			edgeObjlist[i]=Object.create(edgeObj);
			edgeObjlist[i].init(edges[i].nodesEdge[0],edges[i].nodesEdge[1],i);
		}
...
function createGraph(n)
{
	...
	tag.setAttribute("class","title-for-compute");
	tag.appendChild( document.createTextNode("Nodes:"));
	document.getElementById("nodeList").append(tag);
	for(i=0;i<num;i++)
	{
		nodes[i]=alpha[i];
		nodeObjlist[i]=Object.create(nodeObj);
		nodeObjlist[i].init(alpha[i]);
...
function kruskal()// work if input is given in sorted ascendin order
	{
		...
		vertexSets[j].show();
			...
			verSet="";
			for(let b=0;b<vertexSets.length;b++)
				{
				for(let a=0;a<vertexSets[b].v.length;a++)
				{
					verSet+=vertexSets[b].v[a]+" " ;
				}
				verSet="{ "+verSet+"}";
				}
			
			let verTag= document.createElement("label");
				verTag.setAttribute("class","title");
				
				verTag.appendChild(document.createTextNode(verSet));
				let p=document.getElementById(i+"$");
				let c=document.getElementById(i+"$"+"label");
				
				p.replaceChild(verTag,c);
